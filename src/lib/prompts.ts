...
반드시 \`submit_blog_post\` 툴을 호출하여 결과를 제출하세요. 각 필드에 아래 설명에 맞는 값을 채워 넣으세요.
필드 구조와 각 필드의 의미:
{
...
주의: 위 구조는 각 필드의 의미를 설명하기 위한 예시입니다. 실제로는 반드시 submit_blog_post 툴을 호출해 각 필드 값을 전달하세요.
...
function buildRealEstatePrompt(params: GenerateRequest): string {
  ...
  // 매물 소개 주제 여부 — 정보성 가이드가 아니라 특정 매물 중심으로 작성해야 함
  const isListing = (params.realEstateTopic || '').endsWith('_listing');

  const requiredElements = isListing
    ? `## 필수 포함 요소 (⚠️ 매물 소개글 — 일반 정보성 가이드가 아님)
이 글은 **특정 실제 매물 1건을 소개·분석**하는 글입니다. 위 '추가 맥락'에 입력된 실제 매물 정보를 반드시 글의 중심축으로 삼아, 그 매물을 방문/계약하고 싶게 만드는 소개글로 작성하세요. 시장 동향·절차 설명 같은 일반 정보로 흐르지 마세요.
${params.additionalContext
  ? '- 입력된 매물 정보(가격·면적·층·향·특징 등)를 최우선 근거로 사용하세요. 입력되지 않은 수치는 지어내지 말고 일반적 설명으로 자연스럽게 보완하세요.'
  : '- ※ 구체적 매물 정보가 입력되지 않았습니다. 도입부에 "대표 예시 매물"임을 명확히 밝히고, 실제 소개 시 채워야 할 항목(단지명/주소, 가격, 전용/공급면적, 층/향, 준공연도, 관리비 등)을 자연스럽게 안내하세요. 수치를 사실인 것처럼 단정하지 마세요.'}

1. **후킹 도입부**: 이 매물만의 한 줄 매력 포인트 + 위치 첫인상
2. **매물 상세 스펙(핵심)**: 실제 정보를 표/카드로 정리하되 **매물 유형에 맞는 항목**을 중심으로:
   - 주거용(아파트/빌라/오피스텔): 거래유형·가격, 전용/공급면적, 방/욕실 수, 해당층/총층, 향, 구조, 준공연도, 관리비, 옵션, 주차
   - 빌딩/상가: 매매가·보증금/월세, 대지면적·연면적, 층수, 용도지역, 임차 현황·임대수익률, 엘리베이터·주차, 준공연도
   - 공장/창고: 매매/임대가, 대지·건축면적, 층고, 전력 용량, 진입도로·차량 접근, 용도지역, 인허가(폐수·위험물 등)
   - 토지: 매매가·평단가, 지목, 용도지역/지구, 대지면적(㎡/평), 도로 접함 여부, 공시지가, 건폐율/용적률, 개발 가능성
3. **이 매물의 강점**: 유형에 맞는 실제 특징(주거=채광·조망·역세권·학군 / 상업·산업=유동인구·물류 접근성·배후수요 / 토지=개발 호재·입지) 기반으로 구체적으로
4. **입지·주변 환경 분석**: 교통, 배후 수요, 편의·인프라, 개발 호재, 생활·상권/산업 벨트
5. **시세·투자 관점**: 인근 실거래가 대비 가격 적정성, 임대수익률·전월세 비교 또는 개발 시나리오 — 확인 가능한 범위에서
6. **이런 분께 추천 + CTA**: 타겟 실수요/투자자 명시 + 방문·상담 유도, 실거래가 확인 사이트(rt.molit.go.kr) 안내
7. **tags**: 지역명 + 매물유형 + 부동산 키워드 5~8개
8. **imageCards**: 매물 스펙표·강점 요약·입지 개념·시세 비교·체크리스트 등 이 매물 중심의 카드 5장`
    : `## 필수 포함 요소
