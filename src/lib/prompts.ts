...
1. **후킹 도입부**: 이 매물만의 한 줄 매력 포인트 + 위치/단지 첫인상
2. **매물 상세 스펙(핵심)**: 실제 정보를 표/카드로 정리 — 거래유형·가격(매매/전세/월세), 전용/공급면적, 방/욕실 수, 해당층/총층, 향, 구조, 준공연도, 관리비, 옵션, 주차
3. **이 매물의 강점**: 채광·조망·역세권·학군·생활 인프라 등 실제 특징 기반으로 구체적으로
4. **입지·주변 환경 분석**: 교통(지하철/버스), 학군, 편의시설, 개발 호재, 생활권
5. **시세·투자 관점**: 인근 실거래가 대비 가격 적정성, (임대 시) 예상 수익률·전월세 비교 — 확인 가능한 범위에서
